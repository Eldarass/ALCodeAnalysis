...
{
    [DiagnosticAnalyzer]
    class TableFIeldIDValidation : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } = ImmutableArray.Create<DiagnosticDescriptor>(DiagnosticDescriptors.Rule011First19TableFieldsIDReservedToPrimaryKey);

        public override void Initialize(AnalysisContext context)
        {
            context.RegisterSyntaxNodeAction(new Action<SyntaxNodeAnalysisContext>(TableFIeldIDValidation.AnalyzeTableFieldsIDs), SyntaxKind.TableObject);
        }
        ...
        private static void ReportFieldsReservedForPrimaryKeyFields(
           SyntaxNodeAnalysisContext syntaxNodeAnalysisContext,
           Location location,
           string valueText,
           IdentifierNameSyntax syntaxName)
        {
            syntaxNodeAnalysisContext.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule011First19TableFieldsIDReservedToPrimaryKey, location, (object)valueText, (object)syntaxName));
        }
    }
}
