...
{
    [DiagnosticAnalyzer]
   public class ExtensionsFieldsIDValidation : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } = ImmutableArray.Create<DiagnosticDescriptor>(DiagnosticDescriptors.Rule012TableExtensionFieldsNumberedInDedicatedRange, DiagnosticDescriptors.Rule013EnumExtensionsValuesNumberedInDedicatedRange);

        public override void Initialize(AnalysisContext context)
        {
            context.RegisterSyntaxNodeAction(new Action<SyntaxNodeAnalysisContext>(ExtensionsFieldsIDValidation.AnalyzeTableExtFieldsID), SyntaxKind.TableExtensionObject);
            context.RegisterSyntaxNodeAction(new Action<SyntaxNodeAnalysisContext>(ExtensionsFieldsIDValidation.AnalyzeEnumExtValuesID), SyntaxKind.EnumExtensionType);
        }
           ...
           IdentifierNameSyntax syntaxName)
        {
            if (syntaxNodeAnalysisContext.Node != null)
                syntaxNodeAnalysisContext.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule012TableExtensionFieldsNumberedInDedicatedRange, location, (object)valueText, (object)syntaxName));
        }

        private static void ReportEnumValuesInAllowedRange(
           SyntaxNodeAnalysisContext syntaxNodeAnalysisContext,
           Location location,
           string valueText,
           IdentifierNameSyntax syntaxName)
        {
            if (syntaxNodeAnalysisContext.Node != null)
            syntaxNodeAnalysisContext.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.Rule013EnumExtensionsValuesNumberedInDedicatedRange, location, (object)valueText, (object)syntaxName));
        }
    }
}
